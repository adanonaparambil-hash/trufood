...
function Topbar() {
  ...
  return (
    <header
      ...
      style={{
        background: scrolled ? 'rgba(5,8,5,0.92)' : 'rgba(5,8,5,0.6)',
      ...
      <nav className="max-w-[1400px] mx-auto px-8 h-[72px] flex items-center justify-between gap-8">
        ...
        <ul className="hidden lg:flex items-center gap-7">
          {NAV_LINKS.map((l, i) => (
            <li key={l}>
              <Link
                href={l === 'Home' ? '/' : l === 'About Us' ? '/about' : l === 'Products' ? '/products' : l === 'Our Solutions' ? '/technology' : '#'}
                className="relative text-sm font-medium transition-colors duration-200 pb-0.5"
                style={{ color: l === 'About Us' ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.45)' }}
              >
                {l === 'About Us' && (
        ...
        <Link
          href="#"
          className="hidden lg:inline-flex items-center gap-2 text-sm font-medium px-5 py-2.5 rounded-full transition-all duration-200"
        ...
        {/* Mobile menu toggle */}
        <button
          className="lg:hidden flex flex-col gap-1.5 w-8 h-8 justify-center items-end"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden" style={{ background: 'rgba(5,8,5,0.98)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}
          >
            <div className="px-8 py-6 flex flex-col gap-5">
              {NAV_LINKS.map(l => (
                <Link key={l} href={l === 'Home' ? '/' : l === 'About Us' ? '/about' : l === 'Products' ? '/products' : l === 'Our Solutions' ? '/technology' : '#'} className="text-sm" style={{ color: 'rgba(255,255,255,0.6)' }}>{l}</Link>
              ))}
            </div>
          </motion.div>
        )}
